...
Daily aggregator of Florida-located taildragger classified listings,
...
[Delaware](https://github.com/taildraggers/delaware) repos' approach, with
the state filter swapped to "FL".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, FL` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in Florida"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/florida/`
...
```html
<iframe
  src="https://taildraggers.github.io/florida/"
  title="Taildragger Ads in Florida"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_FLORIDA_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
