"""Aggregates Florida-located listings from every taildraggers.com
...
ones whose location ends in ", FL".
...
California/Colorado/Connecticut/Delaware repos' approach, with the state
filter swapped to "FL".
"""
...
PAGE_TITLE = "Taildragger Ads in Florida"
STATE_ABBR = "FL"
...
_FLORIDA_LOCATION_RE = re.compile(r",\s*FL\s*$", re.IGNORECASE)


def _is_florida(location: str) -> bool:
    return bool(_FLORIDA_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        florida_rows = [row for row in rows if _is_florida(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(florida_rows)} in Florida")
        all_listings.extend(florida_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No Florida listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique Florida listings: {len(listings)}")
